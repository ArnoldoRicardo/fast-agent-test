...
from ..database.models import CryptoCurrency, OHLCV
    ...
    async def get_ohlcv_data(self, symbol: str, timeframe: str = "1d", limit: int = 30) -> list:
        ...
        try:
            ...
            if results:
                ...
                self.logger.info(
                    f"No hay datos en la base de datos para {formatted_symbol} {timeframe}, obteniendo de Binance")
            ...
            for pair in self.pairs:
                ...
                for timeframe in self.timeframes:
                    ...
                    if last_ohlcv:
                        ...
                        self.logger.info(
                            f"No hay histórico en la base de datos para {pair} {timeframe}, descargando desde 2020...")
    ...
    async def _fetch_ohlcv_since(self, pair: str, timeframe: str, since: int) -> List[Dict[str, Any]]:
        ...
        try:
            while True:
                ...
                if not ohlcv:
                    self.logger.info(
                        f"Descargadas {len(all_ohlcv)} velas en total para {pair} {timeframe}. No hay más datos.")
                    break
                ...
                self.logger.info(
                    f"Batch {batch}: +{len(ohlcv)} velas (total: {len(all_ohlcv)}) - Última vela: {last_dt}")
