        ...
        try:
            ...
            for coin in data["coins"]:
                ...
                price = CryptoPrice(
                    ...
                    ath_date=datetime.fromisoformat(coin.get("ath_date").replace(
                        "Z", "+00:00")) if coin.get("ath_date") else None,
                    atl_price=coin.get("atl"),
                    atl_date=datetime.fromisoformat(coin.get("atl_date").replace(
                        "Z", "+00:00")) if coin.get("atl_date") else None,
    ...
    async def collect_historical(self, symbol: str, from_timestamp: int, to_timestamp: int) -> Dict[str, Any]:
        ...
        while retry_count <= max_retries:
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, params=params, headers=headers) as response:
                        ...
                        if response.status == 429:
                            ...
                            self.logger.warning(
                                f"Error 429 - Rate limit excedido. Reintentando en {wait_time} segundos (intento {retry_count}/{max_retries})")
                        ...
                        if response.status == 200:
                            ...
                            self.logger.info(
                                f"Se obtuvieron {len(historical_data)} puntos de datos históricos para {symbol}")
                ...
                wait_time = base_delay * (2 ** retry_count)
                self.logger.warning(
                    f"Error al obtener datos. Reintentando en {wait_time} segundos (intento {retry_count}/{max_retries})")
                await asyncio.sleep(wait_time)
